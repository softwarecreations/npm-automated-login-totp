...
const program = ( new commander.Command(packageJson.name)
  ...
  .option('-n --otp-name <npm>', 'Name for the TOTP secret on your YubiKey')
...
const otpName   = getString('YubiKey TOTP Name', 'otpName'  , 'OTPNAME'  ) || 'npm';
...
const getYubiKeyOtpAndLogin = () => {
  ...
  yubiP.stderr.on('data', errData => {
    ...
    if (errS.match(/touch.+?yubi/i)) {
      console.log(colors.green(`Touch your YubiKey to get '${otpName}' OTP`));
      if (!program.verbose) return;
    }
    console.log(colors.red(`YubiKey error said: ${errS}`));
  });
  yubiP.stdout.on('data', yubiData => {
    const yubiS = yubiData.toString();
    if (program.verbose) console.log(`YubiKey said: ${yubiS}`);
    if (yubiS.match(/touch.+?yubi/i)) return console.log(colors.green(`Touch your YubiKey to get '${otpName}' OTP`));
    if (yubiS.match(/\w+\s+(\d{6})/)) {
      const linesA = yubiS.split('\n');
      const otpRegex = yubiS.match(RegExp(`${otpName}\\s+(\\d{6})`));
      if (otpRegex!==null) {
        const otp = otpRegex[1];
        if (!program.quiet) console.log(colors.green(`YubiKey gave '${otpName}' OTP: ${otp}`));
        doNpmLogin(otp);
      } else {
        console.log(colors.red(`YubiKey didn't give OTP name '${otpName}'\nOnly:\n${yubiS}`));
      }
    }
  });
};

if (otpSecret.match(/^yubi/i)) {
  try {
    require('child_process').execSync('ykman --version');
  } catch (err) {
    return exitError("Please ensure that 'ykman' aka 'yubikey-manager' is installed.");
  }
  getYubiKeyOtpAndLogin();
} else {
  doNpmLogin(totp.generate(otpSecret));
}
